...
и убедиться, что в итоговом списке правильные номера интерфейсов и вланов.
...
- trunk_template: ожидает как аргумент шаблон конфигурации trunk-портов в виде списка команд (список trunk_mode_template)
    ...
    for k, v in intf_vlan_mapping.items():
        res.append('interface '+ k)
        for i in trunk_template:
            if i.find('allowed vlan') > -1:
                res.append(i+' '+', '.join(map(str, v)))
            else:
                res.append(i)
    return res

print('Проверка работы функции на примере словаря trunk_config и списка команд trunk_mode_template:')
print( generate_trunk_config(trunk_config, trunk_mode_template) ) 
print('\nПроверка работы функции на примере словаря trunk_config_2 и списка команд trunk_mode_template:')   
